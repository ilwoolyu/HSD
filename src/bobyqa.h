 ...
 * Copyright (c) 2017, Ilwoo Lyu (header version).
 *
...
int min_bobyqa(int n, TYPE *x, Func &func, TYPE xl, TYPE xu, TYPE rb, TYPE tol, int max_iter);
...
int min_bobyqa(int n, TYPE *x, Func &func, TYPE *xl, TYPE *xu, TYPE rb, TYPE tol, int max_iter)
{
	int npt = 2*n+1;
	int ret;
	TYPE *w = (TYPE*)calloc((npt+5)*(npt+n)+3*n*(n+5)/2, sizeof(TYPE));
	ret = bobyqa(n, npt, func, NULL, x, xl, xu, rb, tol, 0, max_iter, w);
	free(w);
	return ret;
}
